...
#Must use tabs in if block below due to -EOF
...
aws_cmd "aws sqs receive-message --region $region --queue-url ${sqsurl} --wait-time-seconds $sqs_wait_time --visibility-timeout $sqs_visibility_timeout" "$aws_cmd_retry_attempts" ||  die "Failed to receive message from SQS! - ${aws_cmd_output}"

if [ -z "$aws_cmd_output" ]; then
  echo "No queue items found."
  echo "Job [$project_name] - [$AWS_BATCH_JOB_ID] on [$AWS_BATCH_JQ_NAME] completed successfully."
  exit 0
fi

sqs_receipt_handle=`echo $aws_cmd_output|jq -r '.Messages[0].ReceiptHandle'`
sqs_message_body=`echo $aws_cmd_output|jq -r '.Messages[0].Body'`
sqs_s3_bucket=`echo "$sqs_message_body"| jq -r '.Records[0].s3.bucket.name'`
sqs_s3_key=`echo "$sqs_message_body"| jq -r '.Records[0].s3.object.key'`
...
if [ $? -ne 0 ]; then
  die "Failed to SFTP to $sftp_endpoint_hostname to transfer $sftp_folder/$sqs_s3_key_file!"
fi

aws_cmd "aws s3 cp $sqs_s3_key_file \"s3://${sqs_s3_bucket}/$sqs_s3_key_file_path/archive/${sqs_s3_key_file}\" --region $region" "$aws_cmd_retry_attempts" || die "Unable to upload to archive on S3! - ${aws_cmd_output}"
aws_cmd "aws s3 rm \"s3://${sqs_s3_bucket}/${sqs_s3_key}\" --region $region" "$aws_cmd_retry_attempts" || die "Unable to remove transferred file from S3! - ${aws_cmd_output}"
aws_cmd "aws sqs delete-message --region $region --queue-url ${sqsurl} --receipt-handle $sqs_receipt_handle" "$aws_cmd_retry_attempts" ||  die "Failed to delete message from SQS! - ${aws_cmd_output}"

if [ "$emailonsuccess" -ne 0 ]; then
  echo "File [$sqs_s3_key_file] transferred successfully for [$project_name] - [$AWS_BATCH_JOB_ID] on [$AWS_BATCH_JQ_NAME]."| mailx -s "[$AWS_BATCH_JQ_NAME] File transferred successfully to $sftp_folder/$sqs_s3_key_file" -r "$emailfrom" -S smtp="$emailserver:$emailserverport" $emailuser
  sleep 20 && echo "Email sent."
fi
